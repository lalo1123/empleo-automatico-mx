...
export async function addToQueue(item) {
  ...
  const normalized = {
    ...
    // Caller-provided appliedAt passes through (markApplied preserves a
    // finite value). Critical for IMPORTED applications: the LaPieza
    // profile importer backdates them so they never count toward TODAY's
    // anti-ban caps — without this, importing 20 old postulaciones would
    // instantly exhaust the 20/day budget.
    appliedAt: Number.isFinite(item.appliedAt) ? Number(item.appliedAt) : undefined
  };
...
/**
 * Normalized "title|company" keys of applied vacancies for a source.
 * Complements appliedIdsForSource: portals re-post the same vacancy under
 * a NEW id (live case: Creditas "Consultor de ventas" and EL CRISOL ×3 on
 * LaPieza), so id matching alone lets already-applied jobs back into the
 * top-N. Key shape must match the panel's normalizer: lowercase,
 ...
 */
/**
 * Canonical posting key: accent-insensitive, punctuation-insensitive.
 * Live bug it fixes: the SAME vacancy renders "Consultor de ventas –
 * Ciudad de México" (en dash) on listing cards but "- " (hyphen) on the
 * profile's Mis postulaciones list — a naive lowercase compare never
 * matches. KEEP IN SYNC with canonPostingKey in content/lapieza.js.
 *
 * @param {string} title
 * @param {string} company
 * @returns {string} "" when both parts are empty
 */
export function postingKey(title, company) {
  const canon = (s) =>
    String(s || "")
      .toLowerCase()
      .normalize("NFD")
      ...
      .trim();
  const t = canon(title);
  const c = canon(company);
  if (!t && !c) return "";
  return t + "|" + c;
}

export async function appliedPostingKeysForSource(source) {
  if (!source) return new Set();
  const queue = await readQueue();
  const keys = new Set();
  for (const q of queue) {
    if (q.source !== String(source) || q.status !== "aplicada") continue;
    const key = postingKey(q.title, q.company);
    if (key) keys.add(key);
  }
  return keys;
}
...
/**
 * Count vacancies the user has applied to TODAY from this source. Used by
 * the bulk auto-postular flow to enforce per-portal daily caps (green-zone
 * thresholds — LinkedIn 15/day, OCC 20/day, etc.) so we don't trip bot
 * detection on the user's account.
 *
 * Counts items whose status === "aplicada" AND appliedAt >= local-midnight.
 * Items without appliedAt (legacy) are NOT counted.
 *
 * @param {string} source
 * @returns {Promise<number>}
 */
export async function countAppliedTodayForSource(source) {
  if (!source) return 0;
  const queue = await readQueue();
  // Local midnight — caps are by calendar day in the user's timezone.
  const d = new Date();
  const startOfToday = new Date(d.getFullYear(), d.getMonth(), d.getDate()).getTime();
  return queue.filter(
    (q) =>
      q.source === String(source) &&
      q.status === "aplicada" &&
      Number.isFinite(q.appliedAt) &&
      q.appliedAt >= startOfToday
  ).length;
}

/**
 * Count ALL vacancies the user has applied to today, across every portal.
 * Used by the bulk gate to enforce the cross-portal cap (AUTO_TOTAL_CAP
 * in schemas.js — default 110/day). Belt-and-suspenders on top of the
 * per-portal cap: prevents a power-user from spreading across 6 portals
 * and accumulating 120+ applications in a day, which triggers "too
 * human" bot-detection heuristics on several platforms.
 *
 * @returns {Promise<number>}
 */
export async function countAppliedTodayAcrossSources() {
  const queue = await readQueue();
  const d = new Date();
  const startOfToday = new Date(d.getFullYear(), d.getMonth(), d.getDate()).getTime();
  return queue.filter(
