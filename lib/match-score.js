...
export function computeMatchScore(profile, jobLite) {
  ...
  if (!hasRichJob) {
    return computeListingMatchScore(profile, jobLite);
  }
...
function computeListingMatchScore(profile, jobLite) {
  const reasons = [];
  ...
  const base = 35;
  const raw = base + skillBonus + roleBonusPts + summaryBonusPts + modalityPts;
  const score = Math.max(0, Math.min(100, Math.round(raw)));
