...
/**
 * Sister-domain pairs. Bucket A and B are "sisters" when a CV in A is
 * a legitimate fit for a job in B (and vice versa). Reasoning:
 *   sales ↔ marketing       — both commercial functions; KAM sells the
 *                              product the brand team positions, vice versa
 *   ops ↔ manufacturing     — supply chain ops and plant ops are adjacent
 *   ops ↔ retail            — store ops, supply chain ops overlap
 *   design ↔ product        — designers ship product, PMs commission design
 *   design ↔ marketing      — creative/branding lives in both
 *   product ↔ tech          — PMs work day-to-day with engineering
 *   finance ↔ legal         — both corporate functions, M&A overlap
 *   hr ↔ admin              — both people/admin operations
 *   customer ↔ sales        — CSMs are post-sale account owners
 *   customer ↔ ops          — support ops overlap
 *   media ↔ marketing       — content/comms shared discipline
 *
 * Bidirectional check via setHas. Symmetric pairs encoded once below;
 * areSisterDomains tries both orderings.
 */
const SISTER_DOMAINS = new Set([
  "sales|marketing",
  "ops|manufacturing",
  "ops|retail",
  "design|product",
...
function areSisterDomains(a, b) {
  if (!a || !b || a === b) return false;
  return SISTER_DOMAINS.has(`${a}|${b}`) || SISTER_DOMAINS.has(`${b}|${a}`);
}
...
// Specific tools / platforms / specialized tech that appear in job titles
// AS THE PRIMARY ROLE REQUIREMENT. If the title mentions one of these AND
// the candidate's CV doesn't have it, the role almost certainly isn't a
// fit — no amount of generic "Analyst" or "datos" overlap should rescue
// a 48% match for a "Splunk Analyst" role from a CV with zero Splunk.
//
// We only check the JOB TITLE (not description) because the title is
// where employers signal the gating tool. Description-level mentions
// (e.g. "experience with Splunk a plus") shouldn't penalize candidates.
//
// All entries lowercase + diacritic-stripped (same shape as normalize()).
const SPECIFIC_TOOLS = new Set([
  // Observability / SIEM / logging — the original bug repro
  "splunk", "datadog", "newrelic", "grafana", "elk", "kibana", "elastic",
  "siem", "qradar", "sentinel", "sumologic", "splunkspl",
  // CRM / sales platforms
  "salesforce", "hubspot", "zendesk", "freshdesk", "dynamics",
  // ERP
  "sap", "oracle", "netsuite", "workday", "peoplesoft",
  // Cloud
  "aws", "azure", "gcp", "kubernetes", "k8s", "terraform", "openshift",
  // Specialized frameworks / langs that gate roles
  "react", "angular", "vue", "svelte", "nextjs", "nuxt",
  "django", "flask", "rails", "laravel", "symfony", "spring",
  "nodejs", "express", "fastapi",
  "kotlin", "swift", "rust", "scala", "elixir", "erlang", "haskell",
  // Data / BI specific tools (NOT generic "sql")
  "tableau", "looker", "qlik", "qliksense", "powerbi",
  "snowflake", "databricks", "redshift", "bigquery", "synapse",
  "airflow", "dbt", "spark", "hadoop",
  // Design
  "figma", "sketch", "invision", "adobexd",
  // Marketing automation
  "marketo", "pardot", "mailchimp", "klaviyo", "braze",
  // Project / dev tools that gate roles
  "jira", "confluence", "atlassian",
  // Security
  "kali", "metasploit", "burpsuite", "wireshark"
]);

// Helper: detect specific tools mentioned in the job title that are NOT
// present in the candidate's CV (skills, role descriptions, or summary).
// Returns array of missing-tool strings.
function detectMissingSpecificTools(titleTokens, profile) {
  const titleTools = [];
  for (const tok of titleTokens) {
    if (SPECIFIC_TOOLS.has(tok)) titleTools.push(tok);
  }
  if (!titleTools.length) return [];

  // Build a "what the user knows" bag: skills + role tokens + summary.
  // We're permissive — any mention anywhere in the CV counts as "knows it".
  const userBag = new Set();
  for (const s of profile?.skills || []) {
    for (const t of tokenize(normalize(s), 30)) userBag.add(t);
  }
  for (const e of profile?.experience || []) {
    for (const t of tokenize(normalize(e?.role || ""), 30)) userBag.add(t);
    for (const t of tokenize(normalize(e?.description || ""), 100)) userBag.add(t);
    for (const ach of e?.achievements || []) {
      for (const t of tokenize(normalize(ach || ""), 50)) userBag.add(t);
    }
  }
  for (const t of tokenize(normalize(profile?.summary || ""), 200)) userBag.add(t);

  return titleTools.filter((tool) => !userBag.has(tool));
}
...
export function computeListingMatchScore(profile, jobLite, preferences = null) {
  ...
  if (profileDomain && titleDomain) {
    ...
    } else if (areSisterDomains(profileDomain, titleDomain)) {
      // Sister buckets — same career neighborhood. A Sales/Marketing CV
      // looking at a "Gerente de Marca" (marketing) shouldn't get a
      // -22 penalty: it's a legitimate adjacent fit. Small bonus
      // recognizes the cross-fit, no penalty.
      domainDelta = 3;
      reasons.unshift(`Dominio cercano: ${titleDomain} (compatible con ${profileDomain})`);
    } else {
      domainDelta = -22;
    ...
    // "Datos Maestros SAP Cadena de Suministro"). Soft-mismatch path
    // checks the title's top-3 buckets:
    //   - If profile domain is in the top-3 → no penalty
    //   - If a SISTER of profile domain is in the top-3 → no penalty
    //     (e.g. profile=sales, title weakly classified as marketing)
    //   - Otherwise → -12 penalty
    const titleTopSet = topDomainsSet(titleCounts, 3);
    if (titleTopSet.size > 0
        && !titleTopSet.has(profileDomain)
        && !Array.from(titleTopSet).some((d) => areSisterDomains(profileDomain, d))) {
      domainDelta = -12;
  ...
  // 6) Specific-tool gate. If the title names a specialized platform
  // (Splunk, Salesforce, SAP, Power BI, React, etc.) and the user's
  // CV has zero mention of it anywhere, that's the strongest negative
  // signal we can detect locally without an LLM. Apply a hard -25 per
  // missing tool (capped at -40) AND a 35% absolute ceiling on the
  // final score — no false 48% matches for "Splunk Analyst" coming
  // from a Marketing CV with "datos" as the only overlap.
  const missingTools = detectMissingSpecificTools(titleTokens, profile);
  let toolGapDelta = 0;
  let toolGapCap = null;
  if (missingTools.length > 0) {
    toolGapDelta = Math.max(-40, -25 * missingTools.length);
    toolGapCap = 35;
    const list = missingTools.slice(0, 2).join(", ");
    reasons.push(
      `Pide ${list} y tu CV no lo menciona — abre la vacante a ver si aplica`
    );
  }

  // Lowered base from 35 to 25 to give the algorithm more discrimination
  // room. With the old base of 35, even a zero-skill-overlap title got
  // ~35-40% which read to the user as "almost half match" — too generous.
  const base = 25;
  ...
  const prefDelta = applyPreferenceBonuses(preferences, jobLite, reasons);
  const raw =
    base + skillBonus + roleBonusPts + summaryBonusPts +
    modalityPts + domainDelta + prefDelta + toolGapDelta;
  let score = Math.max(0, Math.min(100, Math.round(raw)));
  // Apply the tool-gap absolute ceiling AFTER all bonuses. This is the
  // "you don't have the gating tool" hard cap — a Marketing CV will
  // never go above 35% on a Splunk title no matter how many filler
  // tokens overlap.
  if (toolGapCap !== null && score > toolGapCap) {
    score = toolGapCap;
  }
