...
export const MESSAGE_TYPES = Object.freeze({
  ...
  UPLOAD_CV: "UPLOAD_CV",
  // Build a structured profile from a short chat interview (users with NO CV).
  // Payload { qa: [{question, answer}] } → { ok, profile }. Same profile shape
  // as UPLOAD_CV; the options page saves it the same way.
  ...
  // Open the first-install welcome page in a new tab. Content scripts
  // can't chrome.tabs.create — they message the background, which has
  // full extension privileges. Used by the auto-quiz no-CV toast's
  // "Subir CV" action button (lapieza.js) to bypass the
  // ERR_BLOCKED_BY_CLIENT that Chrome throws when a content script tries
  // window.open("chrome-extension://...") directly.
  OPEN_WELCOME: "OPEN_WELCOME",
  // Open a vacancy URL in a BACKGROUND tab (active:false). Used by the
  // bulk auto-postular flow so the user stays anchored in the matches
  // panel watching the inline progress card, not flooded with 5 tabs
  // grabbing focus. Returns { ok, tabId } so the content script can
  // wire a "Ver pestaña" jump-to button per row.
  OPEN_BACKGROUND_TAB: "OPEN_BACKGROUND_TAB",
  // Focus a previously-opened tab by tabId. Called when the user clicks
  // "Ver pestaña" on a bulk-progress row.
  FOCUS_TAB: "FOCUS_TAB",
  // Track a finalized postulación to the backend so the web app can
  // surface a real Historial view. The extension fires this from
  // attachFinalizeApplyTracker (when the user clicks Finalizar) and
  // from the auto-detected "applied" listing-card path. Idempotent
  // server-side (UNIQUE user_id+source+vacancy_id).
  TRACK_APPLICATION: "TRACK_APPLICATION",
  // Append a single event to the application timeline. Fired every
  // time the chain transitions through a meaningful step (cv / cover
  // / questions / quiz / ready / submitted / ...). Server appends to
  // applications.events_json with a 5s same-step dedupe + 50-event
  // cap. The web /account/historial detail drawer reads these to
  // show the per-postulación timeline.
  ...
  // Admin-only: force monthly+daily usage counters. Lets the dev test
  // PLAN_LIMIT_EXCEEDED flows without burning real cuota. Body:
  //   { action: "zero" | "max" }
  // Same FORBIDDEN gate as ADMIN_SET_PLAN.
  ADMIN_SET_USAGE: "ADMIN_SET_USAGE",
  ...
  // "Match real con IA": accurate semantic fit analysis of the profile vs a
  // single vacancy → { score, level, matches[], gaps[], improveTips[], usage }.
  // Has its OWN daily limit (separate from postulaciones); 402 surfaces as a
  // PLAN_LIMIT_EXCEEDED-coded error whose MESSAGE is match-specific.
  ANALYZE_MATCH: "ANALYZE_MATCH",
  // Same as GENERATE_CV but the backend ALSO renders the HTML to an A4
  // PDF via puppeteer-core and streams the binary back. The content
  // script receives the PDF as bytes and creates a File object for
  // programmatic upload to portal file inputs (LaPieza first). This is
  // the "CV optimizado por vacante" feature that justifies Pro/Premium
  // pricing. Quota: 1 unit (same as GENERATE_CV).
  ...
  // Multiple-choice knowledge quiz answer (LaPieza apply forms). The content
  // script detects a quiz container on /apply/<uuid> (e.g. 15-question Power
  // BI test for a Data Analyst position), reads the current question + lettered
  // options (A/B/C/D...), and forwards them ONE AT A TIME — looped per question
  // by the auto-quiz loop in content/lapieza.js. Body shape:
  //   { question: string, options: [{key, text}, ...], profile, job }
  // Response: { ok: true, answerKey: "B" } or { ok: false, error, message }.
  // No quota cost — quiz answers are factual and ride the same Express budget
  // as the cover letter. Errors mirror ANSWER_QUESTIONS (UNAUTHORIZED / 422 /
  // 502). The loop stops on the first error; we don't barrel through with
  // bad answers.
...
export const ERROR_CODES = Object.freeze({
  ...
  // 403 from the backend's emailVerifiedRequired() gate. Distinct from
  // UNAUTHORIZED (401, bad/expired token) so the UI can show a "verify
  // your email" CTA instead of "log in again". Before this existed the
  // 403 fell through mapStatusError to a generic SERVER_ERROR, so the
  // extension couldn't tell the user WHY a cover-letter generation was
  // rejected.
