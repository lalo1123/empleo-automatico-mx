...
export class EmailNotVerifiedError extends BackendError {
  constructor(message) {
    super(
      ERROR_CODES.EMAIL_NOT_VERIFIED,
      message || "Verifica tu correo electrónico antes de continuar.",
      403
    );
...
function mapStatusError(status, serverMessage, serverCode) {
  const msg = serverMessage || "";
  if (status === 401) return new UnauthorizedError(msg);
  if (status === 402) return new PlanLimitError(msg);
  // 403 is the backend's emailVerifiedRequired() gate. Without this case
  // it fell through to the generic SERVER_ERROR below, so the extension
  // showed "El servidor tuvo un problema" instead of telling the user to
  // verify their email — and the cover-letter panel opened blank with no
  // actionable reason. Respect an explicit server code if present (future
  // 403s), else default to EMAIL_NOT_VERIFIED.
  if (status === 403) {
    const err = new EmailNotVerifiedError(msg);
    // Respect an explicit server code if it sent one (future-proof for
    // other 403 reasons); otherwise keep EMAIL_NOT_VERIFIED.
...
async function request(path, opts = {}) {
  ...
  if (!res.ok) {
    ...
    const err = mapStatusError(res.status, srvMsg, srvCode);
...
/**
 * "Match real con IA" — accurate, semantic analysis of how well the profile
 * fits ONE vacancy, plus how to raise it. Metered by its own daily limit
 * (separate from postulaciones). A 402 throws PlanLimitError whose .message is
 * the match-specific copy.
 *
 * @param {{profile: object, job: object}} args
 * @returns {Promise<{ok: true, score: number, level: string, matches: string[], gaps: string[], improveTips: string[], usage?: object}>}
 */
...
/**
 * Same as generateTailoredCv but returns a PDF binary instead of HTML.
 * Backend renders the HTML via puppeteer-core + Alpine chromium.
 *
 * Uses fetch() directly (not the JSON `request` helper) because the
 * response is application/pdf — `await res.json()` would throw. We
 * base64-encode the bytes for transport through chrome.runtime.sendMessage
 * (which can't carry Uint8Array natively). The content-script caller
 * decodes back to bytes and constructs a File for the LaPieza upload.
 *
 * @param {{profile: object, job: object}} args
 * @returns {Promise<{pdfBase64: string, usageCurrent: number|null, usageLimit: number|null}>}
 */
export async function generateTailoredCvPdf({ profile, job }) {
  const token = await auth.getToken();
  if (!token) throw new UnauthorizedError("Necesitas iniciar sesión.");

  let res;
  try {
    res = await fetch(`${API_BASE_URL}/applications/generate-cv-pdf`, {
      method: "POST",
      headers: {
  ...
  if (!res.ok) {
    // Error responses ARE JSON even from this endpoint (sendError).
    let data = null;
  ...
  // Read binary + base64-encode for the chrome.runtime message channel.
  const buf = new Uint8Array(await res.arrayBuffer());
  // btoa on a string of chars in 0-255 range. Chunk to avoid call-stack
  // overflow on >100KB PDFs (default stack ~10k args).
  let binary = "";
  const CHUNK = 0x8000;
  ...
  // Custom headers from the backend surface usage without a /account
  // round-trip — let callers update the cached user record.
  const usageCurrent = Number(res.headers.get("X-EAMX-Usage-Current"));
  const usageLimit = Number(res.headers.get("X-EAMX-Usage-Limit"));

  return {
    pdfBase64,
    usageCurrent: Number.isFinite(usageCurrent) ? usageCurrent : null,
...
/**
 * Sync a finalized postulación to the backend so the web /account/historial
 * page can list it. Server-side ON CONFLICT (user_id, source, vacancy_id)
 * DO NOTHING — safe to call multiple times for the same vacancy.
 *
 * @param {{
 *   source: "lapieza"|"occ"|"computrabajo"|"bumeran"|"indeed"|"linkedin",
 *   vacancyId: string,
 *   url?: string,
 *   title?: string,
 *   company?: string,
 *   location?: string,
 *   matchScore?: number,
 *   status?: "applied"|"viewed"|"rejected"|"hired",
 *   sourceTs?: number,
 *   reasons?: string[]
 * }} input
 */
...
/**
 * Append a single event to an application's timeline. Idempotent
 * server-side (same step within 5s is dropped). Used by the
 * extension's reportBulkStatus to mirror chain progress to the web.
 *
 * @param {{
 *   source: "lapieza"|"occ"|"computrabajo"|"bumeran"|"indeed"|"linkedin",
 *   vacancyId: string,
 *   step: "starting"|"cv"|"cv_personalized"|"cover"|"questions"|"quiz"|"ready"|"submitted"|"error"|"plan_limit"|"closed"|"no_form"|"already_applied",
 *   label?: string,
 *   meta?: object
 * }} input
 */
export async function trackEvent(input) {
  return request("/applications/track-event", {
    method: "POST",
    body: input
  });
}
...
/**
 * Force the caller's monthly + daily usage counters. Used by the
 * extension's options-page admin tools to reproduce PLAN_LIMIT_EXCEEDED
 * flows (action="max") or reset after testing (action="zero") without
 * burning real cuota. Admin-only — backend returns FORBIDDEN for
 * non-admin emails.
 *
 * @param {"zero"|"max"} action
 * @returns {Promise<{ok: true, user: import('./schemas.js').AuthUser, usage: {current: number, limit: number}}>}
 */
export async function setAdminUsage(action) {
  return request("/admin/me/usage", {
    method: "POST",
    body: { action }
  });
}
...
/**
 * Build a structured profile from a short chat interview (for users with no
 * CV document). Same return shape as parseCVText so the rest of the app treats
 * an AI-built profile identically. rawText = the interview transcript.
 * @param {{qa: Array<{question: string, answer: string}>}} args
 * @returns {Promise<object>} a complete UserProfile
 */
export async function buildProfile({ qa }) {
  const data = await request("/applications/build-profile", {
    method: "POST",
    body: { qa }
  });
  const parsed = (data && data.profile) || {};
  const transcript = (Array.isArray(qa) ? qa : [])
    .map((x) => `${x.question}\n${x.answer}`)
    .join("\n\n");
  return {
    version: 1,
